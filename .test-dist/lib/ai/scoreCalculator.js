...
const weightOptimizer_1 = require("./weightOptimizer");
const trendLabels = {
    up: "上昇基調",
    neutral: "中立",
    volatile: "変動大",
...
const DEFAULT_WEIGHTS = (0, weightOptimizer_1.optimizeAiScoreWeights)({
    currentWeights: {
        rsi: 1,
        macd: 1.05,
        ma5: 0.95,
...
function analyzeStock(input, options) {
    ...
    if (candles.length >= 30) {
        ...
        const maStructureScore = (latestClose > latestSma5 ? 8 : -7)
            + (latestSma5 > latestSma25 ? 9 : -5)
            + (latestSma25 > latestSma75 ? 8 : -6);
        const trendMomentumScore = (momentumPersistence >= 0.45 ? 5 : momentumPersistence <= -0.45 ? -5 : 0)
    ...
    const newsCompositeScore = clamp((newsSentimentScaled + newsComponentDelta + newsImpact.score) * learningProfile.newsWeight, -16, 24);
    ...
    const adjustedRawScore = baseScore * baseBlend
        ...
        + regimeAdjustment
        + Math.max(voteBias, -3);
    const adjustedScore = clamp(adjustedRawScore, 0, 100);
    ...
    const blendedRawScore = (productionAiScore * productionBlend
        ...
        + backtestScore * backtestBlend) + (newsClampCarry * adjustedBlend);
    ...
    const weakDowntrendSetup = stock.baselineTrend === "volatile"
