...
import { calculateAtr, calculateVolumeAverage } from "./indicators";
...
function trendStrengthScore(trendStrength?: string) {
...
function newsScore(item: Tepou30Item) {
...
function volumeScore(volumeRatio?: number) {
...
function volatilityScore(volatilityPercent?: number, riskLevel?: "低" | "中" | "高") {
  if (riskLevel === "高") {
    return 42;
  }
...
function riskScore(item: Tepou30Item) {
  return clamp(100 - item.lossRiskPercent * 11, 0, 100);
}

function buildSelectionReason(item: Tepou30Item) {
  const segments = [
...
function dedupeTepou30Items(items: Tepou30Item[]) {
  const unique = new Map<string, Tepou30Item>();
...
async function buildTepou30(timeframe: StockTimeframe, sortMode: Tepou30SortMode) {
  ...
  for (const candidate of finalScoringCandidates) {
    ...
    const volumeAverage = calculateVolumeAverage(candidate.candles, 20);
    const volumeRatio = volumeAverage > 0 ? latestVolume / volumeAverage : 1;
    const atrSeries = calculateAtr(candidate.candles, 14);
