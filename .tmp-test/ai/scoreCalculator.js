...
const expectedValueAnalyzer_1 = require("./expectedValueAnalyzer");
const trendLabels = {
    up: "上昇基調",
    neutral: "中立",
    volatile: "変動大",
...
function mergeRiskLevels(base, expectedValueRisk) {
    const rank = { "低": 0, "中": 1, "高": 2 };
    return rank[expectedValueRisk] > rank[base] ? expectedValueRisk : base;
}
function calcChangePercent(base, latest) {
    if (base <= 0) {
        return 0;
    }
    return ((latest - base) / base) * 100;
}
function calcSlopePercent(values, window = 5) {
...
function calculateRealizedVolatilityPercent(candles, period = 20) {
...
function intradayCalibration(params) {
    const { timeframe, volatilityPercent, volumeSurgeRate, momentumConsistency, trendAlignment, lossRiskPercent, } = params;
...
function buildTrendStack(closes, latestClose) {
...
function classifyTrendDirection(changePercent) {
...
function buildTrendAssessment(trendStack) {
...
function buildGapScore(candles, latestClose) {
    if (candles.length < 2) {
...
function buildVolumeProfile(candles, latestVolume) {
...
function estimateReasonImpact(text) {
    const patterns = [
        [/短期・中期・長期トレンド/, 9],
        [/ニュース重要度/, 9],
...
function buildReasonInsights(reasons, score, signal) {
...
function buildNewsImpact(score, confidence, importance) {
...
function buildAdaptiveWeights(weights, timeframe, candlesLength) {
    if (!timeframe) {
        return { ...weights };
    }
    const w = { ...weights };
...
function decideSignalEnhanced(params) {
    const { score, winRate, expectedValuePercent, riskRewardRatio, marketRegimeScore, trendConsensusScore, } = params;
    ...
    if (expectedValuePercent >= minExpected
    ...
    const sellStrict = expectedValuePercent <= -0.45
        && winRate <= 47
        && riskRewardRatio <= 1.0
        && trendConsensusScore <= 46
        && score <= 44;
    const sellLoose = expectedValuePercent <= -0.8
...
function simulateRollingBacktest(candles, targetPeriodDays = 756) {
    ...
    for (let index = 60; index < lookback.length - 6; index += 1) {
        ...
        const sma5 = (0, indicators_1.calculateSma)(history, 5).at(-1)?.value ?? latest.close;
        const sma25 = (0, indicators_1.calculateSma)(history, 25).at(-1)?.value ?? latest.close;
        const sma75 = (0, indicators_1.calculateSma)(history, 75).at(-1)?.value ?? latest.close;
        const rsi = (0, indicators_1.calculateRsi)(history, 14).at(-1)?.value ?? 50;
        const macd = (0, indicators_1.calculateMacd)(history).at(-1)?.histogram ?? 0;
        const atr = (0, indicators_1.calculateAtr)(history, 14).at(-1)?.value ?? 0;
        const sr = (0, indicators_1.calculateSupportResistance)(history, 20);
        const realizedVolatilityPercent = calculateRealizedVolatilityPercent(history, 20);
        const setupScore = (latest.close > sma5 ? 16 : -10)
            + (sma5 > sma25 ? 14 : -8)
            + (sma25 > sma75 ? 10 : -8)
...
function buildFallbackStock(query) {
    return {
        code: query,
        name: "検索銘柄",
        sector: "未分類",
        baselineTrend: "neutral",
        description: "銘柄マスターにない入力です。実データAPI連携後は検索結果から正式な銘柄情報を取得します。",
    };
}
function analyzeStock(input, options) {
    ...
    if (candles.length >= 30) {
        const sma5 = (0, indicators_1.calculateSma)(candles, 5);
        const sma25 = (0, indicators_1.calculateSma)(candles, 25);
        const sma75 = (0, indicators_1.calculateSma)(candles, 75);
        const rsiSeries = (0, indicators_1.calculateRsi)(candles, 14);
        const macdSeries = (0, indicators_1.calculateMacd)(candles);
        const volumeAverage = (0, indicators_1.calculateVolumeAverage)(candles, 20);
        const bollingerSeries = (0, indicators_1.calculateBollingerBands)(candles, 20, 2);
        const atrSeries = (0, indicators_1.calculateAtr)(candles, 14);
        const adxSeries = (0, indicators_1.calculateAdx)(candles, 14);
        const supportResistance = (0, indicators_1.calculateSupportResistance)(candles, 20);
        const computedVolumeSurgeRate = (0, indicators_1.calculateVolumeSurgeRate)(candles, 20);
    ...
    const backtestRrRaw = oneYearBacktest.totalTrades >= 12
        ...
        : riskRewardRatio;
    const backtestScore = clamp(backtestWinRateRaw * 0.34
        ...
        - backtestDrawdownRaw * 1.75, 0, 100);
    const riskPenalty = lossRiskPercent >= 6 ? 10
        : lossRiskPercent >= 5 ? 7
            : lossRiskPercent >= 4 ? 4
                : lossRiskPercent >= 3 ? 2
                    : 0;
    ...
    const technicalWeighted = (rsiScore * weights.rsi
        ...
        + lossRiskScore * weights.lossRisk) / (weights.rsi
        + weights.macd
        + weights.ma5
        + weights.ma25
        + weights.ma75
    ...
    const adjustedScore = clamp(baseScore * baseBlend
        ...
        + horizonBonus
        + regimeAdjustment
        + Math.max(voteBias, -3), 0, 100);
    const productionAiScore = clamp((backtestWinRateRaw * 0.42
    ...
    const blendedRawScore = (productionAiScore * productionBlend
    ...
    const confidence = clamp(32
        + Math.round(finalScore * 0.36)
        + consensus * 4
        + (candles.length >= 60 ? 6 : 0)
        - (lossRiskPercent >= 5 ? 8 : 0)
        + (riskRewardRatio >= 2 ? 4 : 0), 22, 95) + tradeFactor.confidenceBonus;
    ...
    const impliedWinRateFromEdge = clamp(((backtestExpectedRaw + lossRiskPercent) / Math.max(rewardPercent + lossRiskPercent, 0.0001)) * 100, 10, 96);
    const winRateEdge = clamp((finalScore - 50) * 0.18
        + trendAlignment * 1.1
        + momentumConsistency * 6
        + rrBonus * 0.7
        - lossRiskPercent * 0.75, -12, 12);
    const winRate = clamp(Math.round(baseBacktestWinRate * 0.68 + impliedWinRateFromEdge * 0.32 + winRateEdge), 10, 96);
    const empiricalRiskReward = oneYearBacktest.totalTrades >= 10 ? oneYearBacktest.riskRewardRatio : riskRewardRatio;
    const effectiveRiskRewardRatio = Number(clamp(Math.max(riskRewardRatio, empiricalRiskReward * 0.85), 0.2, 6).toFixed(2));
    let probability5m = clamp(Math.round(winRate * 0.84
        + momentumBias
        ...
        - (lossRiskPercent >= 5 ? 6 : 0)), 15, 86);
    let probability15m = clamp(Math.round(winRate * 0.9
    ...
    const probability1d = clamp(Math.round(winRate
        ...
        - (lossRiskPercent >= 5 ? 6 : 0)), 18, 92);
    const trendConsensusScore = clamp(50
        + trendAlignment * 8
        + maSlopeBlend * 6
        + trendAssessment.totalScore * 1.4
        + momentumConsistency * 16
        + (trendStack.dailyTrend > 0 ? 3 : -3), 0, 100);
    const baseExpectedValuePercent = Number((((winRate / 100) * rewardPercent) - ((1 - winRate / 100) * lossRiskPercent)).toFixed(2));
    const expectedValueAnalysis = (0, expectedValueAnalyzer_1.calculateExpectedValue)({
        aiScore: finalScore,
    ...
    const expectedValuePercent = Number((baseExpectedValuePercent * 0.72 + expectedValueAnalysis.expectedValue * 0.28).toFixed(2));
    const riskLevel = mergeRiskLevels(intrinsicRiskLevel, expectedValueAnalysis.riskLevel);
    ...
    const distributionTargetScore = clamp(50
        + (winRate - 50) * 0.92
        + expectedValuePercent * 7.4
        + (effectiveRiskRewardRatio - 1) * 11.5
        + (trendConsensusScore - 50) * 0.34
        + (marketRegimeScore - 50) * 0.18, 0, 100);
    calibratedFinalScore = clamp(calibratedFinalScore * 0.58 + distributionTargetScore * 0.42, 0, 100);
    const expectedValuePriorityBias = clamp((expectedValueAnalysis.entryPriority - 50) * 0.08, -4, 4);
    calibratedFinalScore = clamp(calibratedFinalScore + expectedValuePriorityBias, 0, 100);
    ...
    const summary = [
        ...
        `損失リスクは${lossRiskPercent}%、1日期待値は${expectedValuePercent}%、リスク水準は${riskLevel}、報酬水準は${expectedValueAnalysis.rewardLevel}、エントリー優先度は${expectedValueAnalysis.entryPriority.toFixed(1)}です。トレンド強度は${trendStrength}です。`,
    ].join(" ");
