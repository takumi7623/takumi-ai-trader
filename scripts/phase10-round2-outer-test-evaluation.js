...
const outerTest = require(path.join(process.cwd(), ".test-dist", "lib", "phase10", "round2OuterTestEvaluation.js"));
...
const REPLAY_IMPLEMENTATION_VERSION = "phase10-round2-outer-test-replay-v1";
...
function round2Rows(candidates) {
...
const rows = round2Rows(candidates);
phase10.assertPreOosRows(rows);
const gate2 = gate1.passed ? round2.gate2Round2CoefficientZero(rows) : { passed: false, skipped: true, reason: "Gate1 failed; Outer Test was not evaluated." };
if (!gate0.passed || !gate1.passed || !gate2.passed) throw new Error("Gate0-Gate2 must pass before Outer Test evaluation");
const result = outerTest.evaluateFrozenRound2OuterTest(rows, folds);

console.log(JSON.stringify({
  gates: { gate0, gate1, gate2 },
  frozenCoefficientPair: outerTest.FROZEN_ROUND2_COEFFICIENT_PAIR,
  coefficientSearchExecuted: result.coefficientSearchExecuted,
  selectionPriorityExecuted: result.selectionPriorityExecuted,
  adoptionDecisionMade: result.adoptionDecisionMade,
  outerTestResult: result,
